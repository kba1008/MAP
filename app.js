...
const GAS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbz5KDMp8KLPyu2WERWNMPpvXy4TEFIUtYISnnkwXpNTRAMF81mBiW3Kpu9QolLykt-d/exec";
...
function bindCheckpointPopup(marker, name, lat, lng, mediaId = '', mediaType = '', desc = '') {
  ...
  if (mediaId && mediaType) {
      ...
      if (mediaType.startsWith('video')) {
          mediaHtml = `
            <div class="relative w-full h-36 mt-2 mb-3 rounded-xl overflow-hidden bg-black flex items-center justify-center border border-slate-200 shadow-sm cursor-pointer group" onclick="openMediaViewer('${uniqueId}', 'video')">
               <div id="${uniqueId}-loader" class="absolute inset-0 flex items-center justify-center text-[11px] text-white bg-slate-800 z-10">${loadingSvg} Memuatkan Video...</div>
               <video id="${uniqueId}" class="hidden w-full h-full object-cover" muted playsinline loop></video>
          ...
          mediaHtml = `
            <div class="relative w-full h-36 mt-2 mb-3 rounded-xl overflow-hidden bg-slate-100 flex items-center justify-center border border-slate-200 shadow-sm cursor-pointer group" onclick="openMediaViewer('${uniqueId}', 'image')">
               <div id="${uniqueId}-loader" class="absolute inset-0 flex items-center justify-center text-[11px] text-slate-600 bg-slate-200 z-10">${loadingSvg} Memuatkan Gambar...</div>
               <img id="${uniqueId}" class="hidden w-full h-full object-cover">
               <div class="absolute inset-0 bg-black/40 hidden group-hover:flex items-center justify-center transition-all z-20">
                   <svg class="w-10 h-10 text-white opacity-90 drop-shadow-lg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path></svg>
               </div>
            </div>`;
  ...
  if (mediaId && mediaType) {
      ...
      marker.on('popupopen', function() {
          ...
          if (el && el.classList.contains('hidden') && !el.dataset.fetching) {
              el.dataset.fetching = "true";
              
              fetch(appendClientParamsToUrl(GAS_WEB_APP_URL + "?action=get_media&media_id=" + mediaId))
                  .then(res => res.json())
                  .then(data => {
                      if (data.status === 'ok') {
                          ...
                          if (loader) loader.innerHTML = `<span class="text-xs text-red-500 font-bold">Gagal memuatkan media</span>`;
                      }
                  })
                  .catch(err => {
                      if (loader) loader.innerHTML = `<span class="text-xs text-red-500 font-bold">Ralat Rangkaian</span>`;
                  });
          }
      });
  }
...
async function confirmCheckpoint() {
  ...
  if (editingCPIndex > -1) {
     ...
     bindCheckpointPopup(cp.marker, name, cp.lat, cp.lng, cp.mediaId, cp.mediaType, cp.desc);
     ...
     bindCheckpointPopup(marker, name, lat, lng, mediaId, mediaType, desc);
     ...
     marker.on('dragend', function(e) {
         ...
         if(cpIndex > -1) {
             ...
             bindCheckpointPopup(marker, checkpoints[cpIndex].name, pos.lat, pos.lng, checkpoints[cpIndex].mediaId, checkpoints[cpIndex].mediaType, checkpoints[cpIndex].desc);
             markUnsavedChanges();
         }
         showToast('Kedudukan dikemaskini', 'info');
     });
     bindMarkerEditEvents(marker);
     checkpoints.push({ name, desc: desc, lat: lat, lng: lng, marker, icon: iconType, iconColor: iconColor, iconSize: iconSize, mediaId, mediaType });
     markUnsavedChanges();
...
function loadEvent(eventId) {
  ...
  eventData.filter(d => d.type === 'checkpoint').forEach(d => {
    ...
    bindCheckpointPopup(marker, d.checkpoint_name, lat, lng, d.media_id, d.media_type, desc);
    ...
    bindMarkerEditEvents(marker);
    checkpoints.push({ name: d.checkpoint_name, desc: desc, lat: lat, lng: lng, marker, icon: iconType, iconColor: iconColor, iconSize: iconSize, mediaId: d.media_id, mediaType: d.media_type });
  });
...
function loadSharedViewerEvent(eventId) {
  ...
  eventData.filter(d => d.type === 'checkpoint').forEach(d => {
    ...
    const desc = d.coordinates || '';
    bindCheckpointPopup(marker, d.checkpoint_name, lat, lng, d.media_id, d.media_type, desc);
    checkpoints.push({ name: d.checkpoint_name, desc: desc, marker, mediaId: d.media_id, mediaType: d.media_type, lat: lat, lng: lng, iconColor: iconColor });
  });
