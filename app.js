...
const GAS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzt8GOO8ONUR8phdCi00t4pW771dwvX4teJl_xMZxhIZith9K8vGQrgey1NcZ3eMjYm/exec";
...
function showContactMasterAdminWhatsApp(reason) {
  const existing = document.getElementById('master-wa-notif');
  if (existing) existing.remove();

  const waMsg = encodeURIComponent(
    'Assalamualaikum Naim, lesen saya ' + (reason || 'bermasalah / telah tamat tempoh') +
    '. Nama pengguna: ' + ((currentUser && currentUser.username) || '-') +
    '. Mohon bantuan untuk lesen baharu. Terima kasih.'
  );
  const waLink = 'https://wa.me/601110661077?text=' + waMsg;
    ...
    <div style="width:100%;max-width:420px;background:linear-gradient(145deg,#0f172a,#1e293b);border:1px solid rgba(239,68,68,0.4);border-radius:20px;padding:24px;box-shadow:0 0 50px rgba(239,68,68,0.15),0 20px 40px rgba(0,0,0,0.5);position:relative;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
        <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#ef4444,#dc2626);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 0 18px rgba(239,68,68,0.4);">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        </div>
        <div>
          <p style="margin:0 0 2px 0;font-size:11px;color:#fca5a5;font-weight:600;letter-spacing:1px;text-transform:uppercase;">Lesen Bermasalah</p>
      ...
      <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:12px;padding:12px;margin-bottom:16px;">
        <p style="margin:0;font-size:12px;color:#fca5a5;line-height:1.6;">\${reason || 'Lesen anda telah tamat tempoh atau sedang digunakan oleh akaun lain.'} Sila hubungi Master Admin untuk mendapatkan lesen baharu atau menyelesaikan isu ini.</p>
      </div>
      ...
      <div style="background:rgba(37,99,235,0.1);border:1px solid rgba(37,99,235,0.3);border-radius:12px;padding:12px 14px;margin-bottom:16px;display:flex;align-items:center;gap:10px;">
        ...
        <div>
          ...
          <p style="margin:0;font-size:12px;color:#93c5fd;font-family:monospace;">📱 011-1066 1077</p>
        </div>
      </div>

      <div style="display:flex;gap:10px;">
        <a href="\${waLink}" target="_blank" rel="noopener"
          style="flex:2;display:flex;align-items:center;justify-content:center;gap:8px;padding:12px;border-radius:12px;background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;font-size:13px;font-weight:700;text-decoration:none;box-shadow:0 4px 14px rgba(22,163,74,0.4);">
        ...
        <button onclick="document.getElementById('master-wa-notif').remove()"
          style="flex:1;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.05);color:#cbd5e1;font-size:13px;font-weight:600;cursor:pointer;">
          Tutup
        </button>
      </div>
    </div>
...
async function fetchMyLicense(){
  ...
  try {
    ...
    if (licenseReadOnly) {
      ...
      showContactMasterAdminWhatsApp(reason);
    }
...
function renderLicenseModal(){
  ...
  if (currentLicense) {
    ...
    if (!currentLicense.valid) {
      const waMsg = encodeURIComponent('Assalamualaikum Naim, lesen saya telah tamat tempoh. Nama pengguna: ' + (currentUser ? currentUser.username : '-') + '. Mohon bantuan lesen baharu. Terima kasih.');
      html += `<a href="https://wa.me/601110661077?text=${waMsg}" target="_blank" rel="noopener" class="mt-2 mb-3 w-full py-2.5 bg-green-600 hover:bg-green-500 rounded text-white text-sm font-semibold flex items-center justify-center gap-2 no-underline" style="display:flex;">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
        WhatsApp Naim — 011-1066 1077
      </a>`;
    }
  } else {
    const waMsg2 = encodeURIComponent('Assalamualaikum Naim, saya tidak mempunyai lesen aktif. Nama pengguna: ' + (currentUser ? currentUser.username : '-') + '. Mohon bantuan. Terima kasih.');
    html += `<p class="text-xs text-red-400 mb-2">Anda belum ada lesen aktif.</p>
      <a href="https://wa.me/601110661077?text=${waMsg2}" target="_blank" rel="noopener" class="mb-3 w-full py-2 bg-green-600 hover:bg-green-500 rounded text-white text-sm font-semibold flex items-center justify-center gap-2 no-underline" style="display:flex;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
        Hubungi Naim — 011-1066 1077
      </a>`;
  }
